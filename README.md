...
# Retrain the diagnosis half of the pipeline.
...
**Ported to `src/`** — ingestion, transformation and the five classifiers. A
full run reproduces the shipped 13-feature list and the shipped Logistic
Regression exactly.

**Still notebook-only** — clustering (Block 5), risk scoring (Block 6), the
SHAP explainer (Block 7) and the recommendation engine (Block 8). Their
artifacts are loaded by the dashboard but are not regenerated by
`train_pipeline`, which is why `--overwrite` leaves them untouched.
...
- **Block 6's save cell is empty.** The `risk_model_*.pkl`, `risk_features.pkl`
  and `repro_features.pkl` files in `artifacts/` cannot be regenerated from the
  notebooks as committed. Treat them as irreplaceable.
